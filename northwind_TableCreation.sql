...
It will reject any INSERT or UPDATE operation that attempts to create a foreign key value in a child table 
if there is no a matching candidate key value in the parent table.

OR

We can set foreign key check as disable before running Query; (I opted for this method)
-- For Session (recommended) - SET FOREIGN_KEY_CHECKS=0;
-- Globally - SET GLOBAL FOREIGN_KEY_CHECKS=0;

Resource - https://stackoverflow.com/questions/21659691/error-1452-cannot-add-or-update-a-child-row-a-foreign-key-constraint-fails
...
/*Error Code: 1452. Cannot add or update a child row: a foreign key constraint fails (`northwind`.`#sql-108c_20`, 
CONSTRAINT `orders_ibfk_2` FOREIGN KEY (`employee_id`) REFERENCES `employees` (`employee_id`)) */


/*Error Code: 3780. Referencing column 'employee_id' and referenced column 'employee_id' 
in foreign key constraint 'orders_ibfk_2' are incompatible.*/
